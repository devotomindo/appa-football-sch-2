...
export type GetCoachRegistrantsBySchoolIdResponse = Awaited<
  ReturnType<typeof getCoachRegistrantsBySchoolId>
>;

export const getCoachRegistrantsBySchoolId = cache(async function (
  schoolId: string,
) {
  ...
  const results = await db
    ...
    .where(
      and(
        ...
        eq(schoolRoleMembers.schoolRoleId, 2),
      ),
    );
